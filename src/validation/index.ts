...
export const PAYMENT_PROVIDER_CHAR_ERROR =
...
const US_STATE_VALUES = new Set(
  [...US_STATES, ...US_TERRITORIES].flatMap((state) => [state.value.toUpperCase(), state.label.toUpperCase()]),
)
...
export function normalizeCountryValue(country?: string | null): string {
...
export function isMatchingBusinessAddress(person: PersonAddress, business: BusinessAddress): boolean {
  if (!person.addressLine1 && !person.city && !person.state && !person.postalCode) return false
  return (
...
export function validateAddressNotMatchingBusiness(
...
const providerString = (max: number) =>
  z
    .string()
    .trim()
    .min(1, 'Required')
...
const optionalProviderString = (max: number) =>
  z
    .string()
    .optional()
 ...
 */
export function makeBusinessProfileSchema(
  options: { requireDbaAndWebsite?: boolean; includeTypeAndDescription?: boolean } = {},
) {
  const typeAndDescription: z.ZodRawShape = options.includeTypeAndDescription
    ? {
        businessType: z
          .string()
          .min(1, 'Required')
...
export const volumeShareByCustomerTypeSchema = z
  ...
  .refine(
    (data) => data.business + data.consumer + data.p2p === 100,
...
export function validateForm(schema: z.ZodTypeAny, data: unknown): StepErrors {
...
export function validateField(schema: z.ZodTypeAny, data: unknown, fieldName: string): string | null {
  const value = (data as Record<string, unknown>)[fieldName]
  if (value === '' || value === undefined || value === null) return null
  const result = schema.safeParse(data)
  if (result.success) return null
  for (const issue of result.error.issues) {
    if ((issue.path[0] as string) === fieldName) return issue.message
  }
  return null
}

/** Returns an error when combined ownership exceeds 100%. */
export function validateOwnershipTotal(owners: { ownershipPercentage?: string }[]): string | null {
