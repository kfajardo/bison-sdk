...
import {
  ...
  type PartialOnboardingAddress,
  type PartialOnboardingErrors,
  type PartialOnboardingValues,
} from '../validation/index.js'
import { el, emit } from './dom.js'
import { readFields, renderFields, setFieldValues, showErrors, type FieldSpec } from './form.js'
...
const INCORPORATION_FIELDS: FieldSpec[] = [
...
const LEADERSHIP_FIELDS: FieldSpec[] = [
...
interface PartialValidation {
  contact: PartialOnboardingErrors
  incorporation: PartialOnboardingErrors
  leadership: PartialOnboardingErrors
  ownership: PartialOnboardingErrors
  owners: PartialOnboardingErrors[]
  consent: PartialOnboardingErrors
}

function checkbox(name: string, text: string): { label: HTMLLabelElement; input: HTMLInputElement } {
...
function address(record: Record<string, string>): PartialOnboardingAddress {
...
function birthDate(value: string): { birthYear?: number; birthMonth?: number; birthDay?: number } {
...
export class BisonOnboardingPartial extends HTMLElement {
  ...
  private bankAccounts?: BankAccountsPanel
  ...
  connectedCallback(): void {
    this.render()
    queueMicrotask(() => { if (this.isConnected) void this.loadBanking() })
  }
  ...
  private render(): void {
    ...
    form.addEventListener('input', (event) => {
      if (event.target instanceof HTMLInputElement) formatInput(event.target)
      this.updateOwnership(form)
      this.updateSubmitState(form, submit)
      const control = event.target as HTMLElement
      if (control.getAttribute('aria-invalid') !== null) this.showFieldError(form, control)
    })
    form.addEventListener('change', () => {
      this.updateOwnership(form)
      this.updateSubmitState(form, submit)
    })
    form.addEventListener('focusout', (event) => this.showFieldError(form, event.target as HTMLElement))
  ...
  private async loadBanking(): Promise<void> {
    const target = this.querySelector<HTMLElement>('.bison-partial__banking')
    if (!target) return
    try {
  ...
  private async showBanking(): Promise<void> {
    const target = this.querySelector<HTMLElement>('.bison-partial__banking')
    if (!target) return
  ...
  private section(step: string, title: string, fields: FieldSpec[]): HTMLElement {
    ...
    section.querySelectorAll<HTMLInputElement>('input[type="date"]').forEach((input) => {
  ...
  private addOwner(
    ...
    form.querySelector<HTMLInputElement>('[name="noOwnersAbove25"]')!.checked = false
    form.querySelector<HTMLInputElement>('[name="ownershipConfirmed"]')!.checked = false
    const owner = this.section('owner', 'Beneficial owner details', OWNER_FIELDS)
    owner.classList.add('bison-partial__owner')
    if (prefill) setFieldValues(owner, prefill)
  ...
  private values(form: HTMLFormElement): PartialOnboardingValues {
    const record = (step: string) => readFields(form.querySelector<HTMLElement>(`.bison-partial__section--${step}`)!)
    ...
    return {
      ...
      ownership: {
        owners: Array.from(form.querySelectorAll<HTMLElement>('.bison-partial__owner')).map((element) => {
  ...
  private validate(form: HTMLFormElement): PartialValidation {
  ...
  private hasErrors(validation: PartialValidation): boolean {
    return [
      validation.contact,
  ...
  private showValidation(form: HTMLFormElement, validation: PartialValidation): void {
    showErrors(form.querySelector<HTMLElement>('.bison-partial__section--contact')!, validation.contact)
    showErrors(form.querySelector<HTMLElement>('.bison-partial__section--incorporation')!, validation.incorporation)
    showErrors(form.querySelector<HTMLElement>('.bison-partial__section--leadership')!, validation.leadership)
    form.querySelectorAll<HTMLElement>('.bison-partial__owner').forEach((owner, index) => {
    ...
    form.querySelector<HTMLInputElement>('[name="termsAccepted"]')
      ?.setAttribute('aria-invalid', validation.consent.termsAccepted ? 'true' : 'false')
  }

  private showFieldError(form: HTMLFormElement, control: HTMLElement): void {
    const field = control.closest<HTMLElement>('.bison-field')
    if (!field) return
    const name = field.querySelector<HTMLInputElement>('.bison-field__input')?.name
    if (!name) return
    const validation = this.validate(form)
    const owner = field.closest<HTMLElement>('.bison-partial__owner')
    let errors: PartialOnboardingErrors
    if (owner) {
      const index = Array.from(form.querySelectorAll('.bison-partial__owner')).indexOf(owner)
      errors = validation.owners[index] ?? {}
    } else if (field.closest('.bison-partial__section--contact')) errors = validation.contact
    else if (field.closest('.bison-partial__section--incorporation')) errors = validation.incorporation
    else errors = validation.leadership
    showErrors(field, { [name]: errors[name] })
  }

  private updateOwnership(form: HTMLFormElement): void {
    const values = this.values(form)
    const hasOwners = values.ownership.owners.length > 0
    const leadershipListed = values.ownership.owners.some(
      (owner) => owner.legalName.trim().toLowerCase() === values.leadership.legalName.trim().toLowerCase(),
    )
    form.querySelector<HTMLElement>('.bison-partial__checkbox-label--noOwnersAbove25')!.hidden = hasOwners
    form.querySelector<HTMLElement>('.bison-partial__checkbox-label--ownershipConfirmed')!.hidden = !hasOwners
    form.querySelector<HTMLElement>('.bison-partial__button--add-leadership')!.hidden =
      !values.leadership.ownsQuarter || leadershipListed
  }

  private updateSubmitState(form: HTMLFormElement, button: HTMLButtonElement): void {
    button.disabled = this.busy || this.complete || this.hasErrors(this.validate(form))
  }

  private setInlineError(form: HTMLFormElement, selector: string, message?: string): void {
  ...
  private setError(message?: string): void {
    const error = this.querySelector<HTMLElement>('.bison-partial__error')
    if (!error) return
    error.textContent = message ?? ''
    error.hidden = !message
  }

  private buildSubmission(values: PartialOnboardingValues): {
    business: BusinessProfilePayload
    officer: ControlOfficerPayload
    owners: BeneficialOwnerPayload[]
  } {
    const officer: ControlOfficerPayload = {
  ...
  private async submit(form: HTMLFormElement, button: HTMLButtonElement): Promise<void> {
    if (this.busy || this.complete) return
    const validation = this.validate(form)
    this.showValidation(form, validation)
    if (this.hasErrors(validation)) return
    ...
    try {
      const status = await this.resolveClient().getOnboardingStatus(this.scope)
      emit(this, 'bison-status-checked', status)
      if (!await this.submitStep('business', { step: 'business', data: payload.business })) return
  ...
  private async submitStep(step: OnboardingStep, submit: OnboardingSubmit): Promise<boolean> {
    try {
