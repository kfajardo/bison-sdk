...
const SECTION_ORDER = ONBOARDING_STEPS;
const SECTION_TITLE = {
...
function statusFor(status, step) {
    switch (step) {
...
function bankingEligible(status) {
...
export class BisonOnboarding extends HTMLElement {
    ...
    set prefill(value) {
        ...
        if (value?.owners?.length)
            this.owners = value.owners.map((o) => ({ ...o }));
        if (this.isConnected)
            this.render();
    }
    get labels() {
    ...
    connectedCallback() {
        if (!this._prefill) {
            const attr = parseJsonAttribute(this, 'prefill');
            if (attr)
                this.prefill = attr;
        }
        if (!Object.keys(this._labels).length) {
            const attr = parseJsonAttribute(this, 'labels');
            if (attr)
                this._labels = attr;
        }
        this.render();
    ...
    uiState(step) {
        ...
        if (!status)
            return step === 'business' ? 'active' : 'locked';
        ...
        if (step !== 'business' && statusFor(status, 'business') !== 'Completed')
            return 'locked';
        if (erroredSteps(status).has(step))
            return 'error';
        if (statusFor(status, step) === 'Completed')
            return 'done';
        return 'active';
    }
    render() {
    ...
    renderSectionCard(step) {
        ...
        if (isOpen) {
            ...
            if (step === 'documents')
                this.renderDocumentsAndBanking(body);
            else
                this.renderFormSection(body, step);
            card.append(body);
        }
        return card;
    }
    renderFormSection(body, step) {
    ...
    redactedPrefill(step) {
        const s = this.status;
        if (!s)
            return undefined;
        const p = {};
        if (step === 'business' && s.taxIdProvided)
            p.ein = '00-0000000';
        if ((step === 'officer' || step === 'owners')) {
            if (s.birthDateProvided)
                p.dateOfBirth = '2000-01-01';
            if (s.governmentIdProvided)
                p.ssn = '000-00-0000';
        }
        return Object.keys(p).length ? p : undefined;
    }
    setFormError(form, message) {
        const errEl = form.querySelector('.bison-onboarding__error');
        if (!errEl)
            return;
        errEl.textContent = message ?? '';
        errEl.hidden = !message;
    }
    async submitSection(step, form, button) {
        if (this.busy)
            return;
        const spec = sectionSpec(step);
        if (!spec)
        ...
        if (!emit(this, 'bison-before-submit', submit, true))
            return;
        this.busy = true;
    ...
    renderDocumentsAndBanking(body) {
        ...
        const uploadBtn = el('button', {
            ...
            onClick: () => { void this.uploadDoc(fileInput, docErr); },
        });
        docWrap.append(el('label', { class: 'bison-field__label', text: 'Verification documents' }), fileInput, uploadBtn, docErr);
        body.append(docWrap);
        ...
        if (businessDone && bankingEligible(this.status)) {
            ...
            if (this.getAttribute('entity-id'))
                bank.setAttribute('entity-id', this.getAttribute('entity-id'));
            bank.client = this.client;
            body.append(bank);
        }
        else {
