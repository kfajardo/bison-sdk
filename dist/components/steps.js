...
import { ALL_US_REGIONS, BUSINESS_TYPES, beneficialOwnerSchema, businessProfileSchema, controlOfficerSchema, digitsOnly, processingVolumeSchema, validateAddressNotMatchingBusiness, validateForm, validateOwnershipTotal, } from '../validation/index.js';
import { el } from './dom.js';
import { readFields, renderFields, setFieldValues, showErrors, slotPlaceholder } from './form.js';
const stateField = { name: 'state', label: 'State', type: 'select', options: ALL_US_REGIONS };
const personFields = [
...
export function sectionSpec(step) {
...
export function renderSection(form, spec, opts = {}) {
    ...
    if (spec.repeat) {
        ...
        owners.forEach((owner, index) => {
            ...
            if (owners.length > 1 && opts.onRemove) {
                fieldset.append(el('button', { type: 'button', class: 'bison-onboarding__button bison-onboarding__button--remove', text: 'Remove', onClick: () => opts.onRemove(index) }));
            }
            form.append(fieldset);
        });
        if (opts.onAdd) {
            form.append(el('button', { type: 'button', class: 'bison-onboarding__button bison-onboarding__button--add', text: spec.repeat.addLabel, onClick: opts.onAdd }));
        }
    }
    else {
        renderFields(form, spec.fields);
        if (opts.prefill)
            setFieldValues(form, opts.prefill);
    }
...
export function collectOwners(form) {
    return Array.from(form.querySelectorAll('.bison-onboarding__repeat-item')).map(readFields);
}
// ── Validation (shared) ───────────────────────────────────────────────────────
/** Validate a single-record section; layers the officer/business-address clash rule. */
export function validateSection(spec, record, business) {
    ...
    if (spec.step === 'officer' && !errors.addressLine1 && business) {
        ...
        if (clash)
            errors.addressLine1 = clash;
    }
    return errors;
}
function addressClash(record, business) {
    return validateAddressNotMatchingBusiness({ addressLine1: record.addressLine1, city: record.city, state: record.state, postalCode: record.postalCode }, { addressLine1: business.addressLine1, city: business.city, state: business.state, zipCode: business.zipCode });
}
...
export function validateOwnersForm(form, spec, business) {
    let valid = true;
    const fieldsets = Array.from(form.querySelectorAll('.bison-onboarding__repeat-item'));
...
export function buildBusinessProfile(r, selectedPaymentMethods, tosToken) {
    return {
...
export function buildControlOfficer(r) {
    return personCore(r);
}
export function buildBeneficialOwner(r) {
    return { ...personCore(r), ownershipPercentage: parseInt(r.ownershipPercentage, 10) };
}
export function buildProcessingVolume(r) {
    const num = (v) => parseInt(digitsOnly(v || '0'), 10) || 0;
    return {
...
export function buildSubmit(step, record, owners) {
    switch (step) {
        case 'business':
...
export class BisonOnboardingStep extends HTMLElement {
    ...
    busy = false;
    get step() {
        return this.getAttribute('step') ?? 'business';
    }
    get persona() {
    ...
    async submit(form) {
        ...
        if (!this.dispatchEvent(new CustomEvent('bison-before-submit', { detail: submit, bubbles: true, cancelable: true })))
            return;
        if (!this.client) {
